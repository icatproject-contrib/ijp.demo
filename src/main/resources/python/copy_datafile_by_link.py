...
# Copy a datafile to another dataset - demo script; using IDS getLink
# usage: copy_datafile_by_link options
...
try:
    filePath = session.idsClient.getLink(sessionId,datafileId)
...
try:
    dfid = session.writeDatafile(filePath, targetDataset, filename, fileFormat, fileDesc, doi, createTime, modTime)
